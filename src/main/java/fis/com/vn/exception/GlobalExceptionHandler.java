...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(LCPlatformException.class)
    public ResponseEntity<?> handleLCPlatformException(LCPlatformException ex) {
        ex.printStackTrace();
        return ResponseFactory.error(ex.getCode(), ex.getMessage(), ex.getData());
    }
    ...
    public ResponseEntity<?> constraintViolationException(HttpMessageNotReadableException ex) {
        ...
        // special case of JsonMappingException below, too much class detail in error messages
            ...
            if (mie.getPath() != null && mie.getPath().size() > 0) {
                msg = Translator.toLocale(mie.getPath().get(0).getFieldName()+".invalid");
            }
